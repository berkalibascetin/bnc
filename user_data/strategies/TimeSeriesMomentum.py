...
"""TimeSeriesMomentum — backtest-only candidate. Not for live."""
...
class TimeSeriesMomentum(RiskSizingMixin, IStrategy):
    INTERFACE_VERSION = 3
    can_short = False
    timeframe = "1d"
    ...
    lookback = IntParameter(10, 120, default=60, space="buy", optimize=False, load=True)


    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        lb = int(self.lookback.value)
        dataframe["mom"] = dataframe["close"] / dataframe["close"].shift(lb) - 1.0
        dataframe["atr"] = ta.ATR(dataframe, timeperiod=14)
        dataframe["atr_pct"] = atr_pct_series(dataframe, dataframe["atr"])
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        cond = (dataframe["mom"] > 0) & (dataframe["mom"].shift(1) <= 0) & (dataframe["volume"] > 0)
        dataframe.loc[cond, ["enter_long", "enter_tag"]] = (1, "tsmom")
        return dataframe
