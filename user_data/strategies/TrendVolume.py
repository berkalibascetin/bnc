...
"""TrendVolume — backtest-only candidate. Not for live."""
...
class TrendVolume(RiskSizingMixin, IStrategy):
    INTERFACE_VERSION = 3
    can_short = False
    timeframe = "1d"
    ...
    vol_len = IntParameter(10, 40, default=20, space="buy", optimize=False, load=True)


    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["ema"] = ta.EMA(dataframe, timeperiod=int(self.ema_len.value))
        dataframe["vol_ma"] = dataframe["volume"].rolling(int(self.vol_len.value)).mean()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[cond, ["enter_long", "enter_tag"]] = (1, "trend_vol")
        return dataframe
